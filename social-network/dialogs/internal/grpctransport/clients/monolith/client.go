...
import (
	...
	"github.com/shaelmaar/otus-highload/social-network/dialogs/internal/grpctransport/clients/interceptors"
...
type Client struct {
	...
	AuthService monolithGRPC.AuthServiceV1Client
}

type GRPCErrors interface {
	monolithGRPC.ValidateTokenReply_ERR_INFO_REASON
	String() string
}
...
func NewGRPCClient(opts *NewClientOptions) (*Client, error) {
	...
	unaryInter := []grpc.UnaryClientInterceptor{
		interceptors.UnaryClientMetadata("social-network-dialogs"),
	...
	client.AuthService = monolithGRPC.NewAuthServiceV1Client(clientConn)
