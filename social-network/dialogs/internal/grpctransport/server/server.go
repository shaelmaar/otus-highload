...
import (
	...
	"github.com/shaelmaar/otus-highload/social-network/dialogs/internal/grpctransport/server/interceptors"
)

type GRPCHandlers interface {
	CreateMessage(context.Context, *servergrpc.CreateMessageRequest) (*servergrpc.CreateMessageReply, error)
	GetDialogMessages(
		context.Context, *servergrpc.GetDialogMessagesRequest) (*servergrpc.GetDialogMessagesReply, error)
}
...
func New(opts *NewServerOptions) (*Server, error) {
	unaryInter := unaryInterceptors(opts.Logger, "social-network-dialogs")
	...
	//nolint:exhaustruct // UnsafeDialogsServiceV1Server эмбеддинг интерфейса.
	servergrpc.RegisterDialogsServiceV1Server(grpcServer, &gRPCDialogsServiceService{Server: s})
...
type gRPCDialogsServiceService struct {
	servergrpc.UnsafeDialogsServiceV1Server
	Server
}

func (s *gRPCDialogsServiceService) CreateMessage(
	ctx context.Context, req *servergrpc.CreateMessageRequest) (*servergrpc.CreateMessageReply, error) {
	if err := validate(s.Validator, req, servergrpc.CreateMessageReply_VALIDATION_ERROR); err != nil {
		return nil, err
	}

	return s.Handlers.CreateMessage(ctx, req)
}

func (s *gRPCDialogsServiceService) GetDialogMessages(
	ctx context.Context, req *servergrpc.GetDialogMessagesRequest) (*servergrpc.GetDialogMessagesReply, error) {
	if err := validate(s.Validator, req, servergrpc.GetDialogMessagesReply_VALIDATION_ERROR); err != nil {
		return nil, err
	}

	return s.Handlers.GetDialogMessages(ctx, req)
}

type GRPCErrors interface {
	servergrpc.CreateMessageReply_ERR_INFO_REASON | servergrpc.GetDialogMessagesReply_ERR_INFO_REASON
	String() string
}
